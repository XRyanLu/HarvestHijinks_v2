...
public:
     ...
     *
     * Loads a sound effect file stored in ADPCM IMA mono format and creates a
     * sound spec that can be used to play the sound effect
     *
 ...
 * @brief Audio manager
 *
 * The audio manager is responsible for management of sound effects, update of
 * the audio system and play back of sound effects and background music.
 *
 * Example usage:
 * @code
...
public:
     ...
     *
     * This should be called each main loop update to ensure that sound effects
     * are played back consistently.
     */
    void        Update();
     ...
     *
     * The sound effect will be added to the audio managers list of loaded sound
     * effects. If the sound effect already exists within the audio manager then
     * no sound effect will be loaded. Its useful to preload larger sound
     * effects during app set-up to prevent delays when playing audio back
     * during game play.
     *
     ...
     *
     * Once a suond effect has been played it will be cached in the audio
     * manager so future play back will not be hindered by having to re-load the
     * sound effect from storage.
     *

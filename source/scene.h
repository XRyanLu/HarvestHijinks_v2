 ...
 *
 * Scenes act as containers that store an hierarchy of CNodes. A scene also
 * contains:
 * - A tweens manager - Enables the scene to create scene local tweens that are
 * updated with the scene
 * - A timers manager - Enables the scene to create scene local timers that are
 * updated with the scene
 *
 * Note that the Init(), Update() and Render() methods are meant to be overidden
 * in your own scene class that is derived from Scene to provide functionality
 * and rendering that is specific to your own scene class.
 *
 */
 ...
 * and rendering of those scenes and provides a mechanism to easily switch
 * between them.
 *
 * Example usage:
 * @code
 ...
 * @endcode
 *
 * The scene manager uses teh concept of the "current scene" where only one
 * scene can be active at any given time. To change from one scene tp another
 * you use the SwitchTo() method.
 *
 * Note that when a scene is not currently the active scene, all tweens and
 * timers within the scene will pause execution.
 *
 */
class SceneManager
{
